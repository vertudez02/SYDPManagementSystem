...
?>
		...
		<div id="content" class="content">
			...
			<!-- begin page-header -->
			<h1 class="page-header">List of Scholar</h1>
			<!-- end page-header -->
			...
			<div class="panel panel-inverse">
                        ...
                        <div class="panel-heading" style="background-color:rgb(16,53,130);">
                           ...
                           <h4 class="panel-title">Scholar</h4>
                        </div>
                        <!-- end panel-heading -->
                        ...
                        <div class="panel-body">
                            <table id="data-table-select" class="table table-striped table-bordered">
                                ...
                                <tbody>
                                    ...
                                    <?php
                                        $view_query = mysqli_query($connection,"SELECT * FROM `t_scholar_student1_profile` p INNER JOIN `r_university` a ON a.univ_id=p.univ_id INNER JOIN `r_course` b ON b.course_id=p.course_id INNER JOIN `r_scholar` c ON c.scholar_id=p.scholar_id INNER JOIN `t_scholar_student2_personalinfo` d ON d.scholar_profile_no=p.scholar_profile_no INNER JOIN `t_scholar_student5_batchyear` e ON e.scholar_profile_no=p.scholar_profile_no INNER JOIN `r_semester` f ON f.semester_id=e.semester_id where scholar_profile_status ='Continue' AND semester_status = 'Active' ");
                                        while($row = mysqli_fetch_assoc($view_query))
                                        {
                                            ...
                                            $scholar_personal_address = $row["scholar_personal_address"];
                                            $scholar_type = $row["scholar_type"];
                                            $univ_code = $row["univ_code"];
                                            $course_code = $row["course_code"];
                                            $scholar_profile_studyear = $row["scholar_profile_studyear"];
                                            $scholar_batch_year = $row["scholar_batch_year"];
                                    ...
                                    ?>   
                                    <tr>
                                        <td hidden><?php echo $scholar_profile_id; ?></td>
                                        <td width=""><?php echo $scholar_profile_no; ?></td>
                                        <td width=""><?php echo $studentname; ?></td>
                                        <td width=""><?php echo $scholar_personal_address; ?></td>
                                        <td width=""><?php echo $scholar_type; ?></td>
                                        <td width=""><?php echo $univ_code; ?></td>
                                        <td width=""><?php echo $course_code; ?></td>
                                        <td width=""><?php echo $scholar_profile_studyear; ?></td>
                                        <td width=""><?php echo $scholar_batch_year; ?></td>
                                        <td width=""><?php echo $semester_description; ?></td>
                                        <td width=""><span class="label label-primary"><?php echo $scholar_profile_status; ?><span></td>
                                       <td class="text-nowrap">

                                         <a data-toggle="modal" href="#edit<?php echo $scholar_profile_id?>" class="btn btn-warning">
